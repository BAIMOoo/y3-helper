    ...
    switch (message.type) {
        ...
        case 'OPEN_EXTENSION_SETTING_AUTHORIZATION_PATH': {
            vscode.commands.executeCommand(
                'workbench.action.openSettings', 'CodeMaker'
            );
            return true;
        }
        ...
        case 'CONSOLE_ERROR': {
            if (Array.isArray(message.data)) {
                console.error('[CodeMaker WebView]', ...message.data);
            }
            return true;
        }
        ...
        case 'PRINT_LOG': {
            if (Array.isArray(message.data)) {
                console.log('[CodeMaker WebView]', ...message.data);
        ...
        case 'CONSOLE_WARN': {
            if (Array.isArray(message.data)) {
                console.warn('[CodeMaker WebView]', ...message.data);
            }
            return true;
        }
...
async function handleOpenFile(data: any) {
    ...
    try {
        ...
        console.error('[CodeMaker] OPEN_FILE error:', err);
    }
}
...
async function handleCreateFile(data: any) {
    ...
    try {
        ...
        console.error('[CodeMaker] CREATE_FILE_AND_INSERT_CODE error:', error);
    }
}
...
async function handlePreviewDiffCode(data: any, provider: CodeMakerWebviewProvider) {
    ...
    try {
        ...
        console.error('[CodeMaker] PREVIEW_DIFF_CODE error:', err);
    }
}
...
async function handleEditCodebaseRules() {
    try {
        ...
        const filePath = path.join(
            workspaceFolder.uri.fsPath, '.codemaker.codebase.md'
        );
...
async function handleGetRules(provider: CodeMakerWebviewProvider, panelId?: string) {
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    if (!workspaceFolder) {
    ...
    const rulesDir = path.join(workspaceFolder.uri.fsPath, '.codemaker', 'rules');
    try {
    ...
    // 2. 加载 .codemaker.codebase.md（特殊硬编码文件）
    const codebaseMdPath = path.join(workspaceFolder.uri.fsPath, '.codemaker.codebase.md');
    try {
