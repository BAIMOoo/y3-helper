...
CodeMaker 集成到 Y3Helper 后（[#98275](https://up1.pm.netease.com/v6/issues/98275)），WebView 前端与 Extension 之间的消息通道存在大量未处理的消息类型，导致多个核心功能不可用：`@` 附加文件卡 loading、AI 无法读写本地文件、聊天发送后无响应、Rules/MCP 配置不生效等。原始设计低估了 Extension 侧需要处理的消息量（源码版有 96 个 case，集成版仅实现了 8 个）。
